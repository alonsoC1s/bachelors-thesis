...
\begin{algorithm}
    ...
    \Function{\Fit{$f_\L$}}{
        ...
        \While{$S$ is not empty}{
            $t, \, \L_t \gets S$.\Pop{} \;
            \uIf{stopping criteria are not met}{
...
\begin{algorithm}
    \SetKwFunction{BestSplit}{FindBestGreedyBinarySplit}
    \KwIn{A subset $\L_t$ of the training set $\L$}
    \KwOut{The best binary, greedy, split $s_*$ on $\L_t$}
    \Function{\BestSplit{$\L_t$}}{
        $\Delta \gets - \infty$ \;
        \For{$j \gets 1, \dots, p$}{
            Find the best greedy binary split $s_{*}^{j}$ that defines node $t$ for the feature $X_j$ \;
