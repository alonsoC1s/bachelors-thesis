...
\begin{algorithm}
    ...
    \Function{\Predict{$f_\L, \vec{x}$}}{
        ...
        \While{$t$ is not a terminal node}{
            $q \gets$ the question in $\mathcal{Q}$ that defines node $t$.
            $t \gets$ the child node $t'$ of $t$ that corresponds to evaluating $\vec{x}$ at question $q$  \;
        }
...
\begin{algorithm}
    ...
    \Function{\GreedyBestSplit{$\L_t$}}{
        $\Delta \gets - \infty$ \;
        \For{$j \gets 1, \dots, p$}{
            Find the best greedy binary split $s_{*}^{j}$ defining node $t$ for the feature $X_j$ \;
