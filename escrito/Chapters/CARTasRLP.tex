...
\begin{array}{rl@{}ll}
    \displaystyle \min_{i_G: B_\L \to \R} & \displaystyle \sum_{t \in B_\L} i_{G}(t) \\
    \text{Subject to} & \displaystyle i_{G}(\bar{s}) \geq \Delta i_{G}(\bar{s}, t') + \sum_{t'} i(t') & \quad \forall t' \in B_\L, \bar{s} \in \mathcal{Q}. \\
...
\]
We also introduce $\vec{Di}_{\bar{s}} \in \R^{|B_\L|}$ defined as $[\Delta i(t_1, \bar{s}), \dots, \Delta i(t_{|B_\L|}, \bar{s})]^{\top}$.
...
\begin{array}{rl@{}ll}
    ...
    \text{Subject to} & \displaystyle \vec{i} \geq \vec{Di}_{\bar{s}} + \gamma \sum_{t'} \vec{i} & \quad \forall \bar{s} \in \mathcal{Q}, \\
    & \vec{i} \text{ unconstrained.} &
\end{array}
\end{equation}
This formulation is nothing more than the original ELP using the equivalences
between state-value and rewards functions established this chapter.
