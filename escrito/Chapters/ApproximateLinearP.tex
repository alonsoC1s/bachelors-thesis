...
With our toolbox almost complete, it is time to advance in our search for
...
scope, and the literature for those techniques is excellent. For a more detailed
...
A much more interesting and aesthetically pleasing way of proving theorem \ref{thrm:opt-policy-existence} is by using the properties of Bellman's operators
...
First, we start by noting that, by Theorem 6.2.2a in
\cite[Ch.~6.9.1]{puterman2014}, whenever a given policy $\vec{v}$ satisfies
\begin{equation*}
...
observed values $\vec{x}_i$. The key idea here is, that these parameters are obtained
...
The next chapters of this part are dedicated to briefly reviewing the extensions
